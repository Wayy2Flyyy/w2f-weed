...
--- Resolve a human-readable label for a granted item (ox_inventory label first,
--- then strain catalogue, then the raw item name).
---@param name string
---@return string
local function rewardItemLabel(name)
    local ok, item = pcall(function() return exports.ox_inventory:Items(name) end)
    if ok and type(item) == 'table' and item.label then
        return item.label
    end
    local strain = W2FWeed and W2FWeed.Strains and W2FWeed.Strains.GetBySeedItem(name)
    if strain and strain.label then
        return strain.label .. ' Seeds'
    end
    return name
end

--- Build a short "2x Skunk Seeds, 1x Exotic Seeds" summary of a reward grant.
---@param grantedItems table[] list of { name = string, amount = number }
---@return string|nil
function W2F.FormatGrantedItems(grantedItems)
    if type(grantedItems) ~= 'table' or #grantedItems == 0 then return nil end
    local parts = {}
    for _, entry in ipairs(grantedItems) do
        parts[#parts + 1] = ('%dx %s'):format(entry.amount or 1, rewardItemLabel(entry.name))
    end
...
---@return table grantedItems list of { name, amount } actually granted
function W2F.GrantSeedReward(src, citizenid, tier)
    ...
    return #grantedItems > 0, grantedItems
end
