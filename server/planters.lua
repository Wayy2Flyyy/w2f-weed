...
--- Roll bud count for harvest (global range + optional bias toward high yields).
---@return integer
local function rollHarvestBudCount()
    local hb = Config.Growth and Config.Growth.HarvestBud
    local lo = (hb and tonumber(hb.Min)) or 12
    local hi = (hb and tonumber(hb.Max)) or 28
    lo = math.floor(lo)
    hi = math.floor(hi)
    ...
    -- max-of-two uniform: favours the upper part of the range
    return math.max(math.random(lo, hi), math.random(lo, hi))
end
...
function W2F.SavePlanter(p)
    ...
    MySQL.insert.await([[
        INSERT INTO w2f_weed_planters
            (planter_id, citizenid, item, model, state, x, y, z, heading, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        p.id, p.owner, p.item, p.model, p.state,
...
function W2F.UpdatePlantProgress(plant)
    ...
    MySQL.update.await([[
        UPDATE w2f_weed_plants
            SET progress = ?, stage = ?, status = ?, last_tick = ?
            WHERE plant_id = ?
    ]], {
        plant.progress, plant.stage, plant.status or 'growing', plant.lastTick or W2F.GetTimestamp(),
        plant.id,
    })
end
...
RegisterNetEvent('w2f-weed:server:addSoilToPlanter', function(planterId)
    ...
    DebugP('soil added to', planterId)
    notify(src, 'SoilAdded', 'success')
end)
...
RegisterNetEvent('w2f-weed:server:harvestPlant', function(plantId)
    ...
    local count = rollHarvestBudCount()
...
RegisterNetEvent('w2f-weed:server:applyFertilizerToPlant', function(plantId, fertilizerItem)
    ...
    DebugP('fertilizer applied', fertilizerItem, 'to', plantId, '+', gain, 'progress')
...
local function applyGrowthDelta(plant, deltaSeconds)
    if not plant or plant.status ~= 'growing' then return end
    if plant.progress and plant.progress >= 100 then return end
    ...
    -- Fraction of tick that has elapsed in real time
    local fraction = deltaSeconds / (tickMinutes * 60)
    if fraction <= 0 then return end

    local gain = avg * mult * fraction
    -- Add randomness within bounds for the "online" path; keep catch-up smooth
