...
--- Rolling is active only when the processing master flag AND the module flag are on.
...
lib.callback.register('w2f-weed:server:rollingMinigameReward', function(source, payload)
    ...
    --- Quality is produced by the client minigame, so sanitise it server-side
    --- (clamp to 0-100, reject garbage) before it can influence rewards.
    local quality = tonumber(payload.quality)
    if quality == nil then
        return { ok = false, reason = 'quality' }
    end
    quality = W2F.Clamp(quality, 0, 100)
    ...
    --- High-quality rolls can yield a bonus joint from the same materials.
    local bonusCfg = Config.RollingMinigame.BonusJoint
    if bonusCfg and quality >= (tonumber(bonusCfg.Threshold) or 101) then
        local chance = tonumber(bonusCfg.Chance) or 0
        if chance > 0 and math.random(100) <= chance
            and exports.ox_inventory:CanCarryItem(src, jointItem, 1)
            and exports.ox_inventory:AddItem(src, jointItem, 1) == true then
            return { ok = true, joint = jointItem, quality = quality, bonus = true }
        end
    end

    return { ok = true, joint = jointItem, quality = quality }
end)
