...
function W2F.ProcessTipPayment(src, citizenid)
    ...
    return W2F.RemoveMoney(src, Config.Payment.Account, price, 'w2f-weed shipment tip')
end
...
function W2F.ProcessBribe(src, citizenid, option)
    ...
    -- Per-tier cooldown key: each bribe option defines its own cooldown
    -- (small/medium/large). Sharing one key let a short cooldown unlock a
    -- longer-cooldown tier early.
    local cooldownKey = citizenid .. ':bribe:' .. tostring(option)
