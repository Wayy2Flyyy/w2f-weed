...
W2FWeed = W2FWeed or {}
W2FWeed.PlacementCursor = W2FWeed.PlacementCursor or {}

do
    local PlacementCursor = W2FWeed.PlacementCursor
    ...
    function PlacementCursor.FromCamera(distance, ignoreEntity)
        distance = distance or 8.0
        local ped = cache and cache.ped or PlayerPedId()
        ignoreEntity = ignoreEntity or ped

        if lib and lib.raycast and lib.raycast.fromCamera then
            local hit, entityHit, endCoords = lib.raycast.fromCamera(511, 4, distance)
            if hit == 1 and endCoords then
                if entityHit and entityHit ~= 0 and entityHit == ignoreEntity then
                    return false, nil
                end
    ...
    function PlacementCursor.SnapToGround(x, y, z, requireGround)
        if requireGround == false then
            return vec3(x, y, z)
        end
...
local function showHints(itemLabel)
    ...
    local text = table.concat({
        ('**%s**'):format(Locale('placeable_hint_title', itemLabel or 'Item')),
        Locale('hint_aim_prop'),
...
local function attachTarget(entity, payload)
    ...
    local options = {}
    local itemCfg = Placeables.GetConfig(payload.item)

    -- "Use" station option (e.g. weed bench -> Rolling Station). Listed first so
    -- the primary action on a work bench is to use it, not pick it up.
    if itemCfg and itemCfg.openRolling and W2F and W2F.OpenRollingCraft then
        options[#options + 1] = {
            name     = 'w2f_weed_use_' .. payload.id,
            icon     = 'fa-solid fa-joint',
            label    = Locale('target_bench_use'),
            distance = 2.0,
            onSelect = function()
    ...
    if payload.pickupEnabled ~= false then
        options[#options + 1] = {
            ...
            label    = Locale('target_placeable_pickup', payload.label or payload.item),
            distance = 2.5,
            onSelect = function()
                if InteractionBusy then return end
                InteractionBusy = true
                local ok = doProgress(Locale('progress_pickup_object'), (Config.Placeables.Progress and Config.Placeables.Progress.PickupObject) or 2500, {
                    dict = 'pickup_object',
                    clip = 'pickup_low'
                })
...
local function getGroundCoords(x, y, z)
    return W2FWeed.PlacementCursor.SnapToGround(x, y, z, Config.Placeables.Placement.RequireGround)
end
...
local function startPlacement(itemName, slot)
    ...
    local rayDist = placementCfg.CursorRayDistance or 8.0
    local previewCoords = vec3(
        initialSnapped.x,
        initialSnapped.y,
        initialSnapped.z + (cfg.zOffset or 0.0) + (placement.heightOffset or 0.0)
    )
    ...
    CreateThread(function()
        while PlacementActive and CurrentPreview ~= 0 and DoesEntityExist(CurrentPreview) do
            ...
            local hit, hitCoords = W2FWeed.PlacementCursor.FromCamera(rayDist, CurrentPreview)
            ...
            SetEntityCoordsNoOffset(CurrentPreview, previewCoords.x, previewCoords.y, previewCoords.z, false, false, false)
            SetEntityHeading(CurrentPreview, CurrentHeading)

            local valid = checkValid(previewCoords)
            ...
            if IsDisabledControlJustReleased(0, controls.Place) then
                if not CurrentValid then
                    ...
                    InteractionBusy = true
                    local ok = doProgress(Locale('progress_place_object'), (Config.Placeables.Progress and Config.Placeables.Progress.PlaceObject) or 2500, {
