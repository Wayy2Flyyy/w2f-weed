...
--- Current search progress for the active shipment, for the zone prompt.
---@return integer done
---@return integer|nil required
function W2F.GetShipmentProgress()
    if not activeShipment then return 0, nil end
    return activeShipment.searchesDone or 0, activeShipment.searchesRequired
end
...
RegisterNetEvent('w2f-weed:client:shipmentSearchProgress', function(data)
    ...
    if W2F.RefreshSearchTextUI then W2F.RefreshSearchTextUI() end
end)
...
RegisterNetEvent('w2f-weed:client:shipmentCompleted', function(summary)
    local desc = Locale('shipment_completed')
    if type(summary) == 'string' and summary ~= '' then
        desc = Locale('shipment_reward_summary', summary)
    end
    lib.notify({ title = 'Shipment', description = desc, type = 'success', duration = 7000 })
    W2F.ClearShipmentVisuals()
end)
